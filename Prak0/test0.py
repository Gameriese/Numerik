...
class Test_Praktikum(Test_Numerik):
    ...
    def test_hallo(self):
        with self.subTest(msg='main.hallo.hallo nicht korrekt oder gar nicht definiert.'):
            self.runner(lambda x: main.hallo.hallo, (None, ), self.assertEqual)
        with self.subTest(msg='Die Versionsnummer ist nicht korrekt.'):
            self.runner(lambda x: main.hallo.version, (None, ), self.assertEqualStringBeginning, true_value=['3.6.', '3.7.', '3.8.'])
                    
#    def test_binom(self):
#        for n, k in ((10, 2), (2, 0), (0, 0), (10**10, 10), (10**10, 10**10 - 10)):
#            with self.subTest(msg='Berechnung von binom(n, k) nicht korrekt für n = {} und k = {}.'.format(n, k)):
#                self.runner(main.binom.binom, (n, k), self.assertAlmostEqualRelative)

    def test_mitternacht(self):
        for p, q in ((-3, 2), (3, 2), (-10.0, 10**-30), (10.0, 10**-30)):
            with self.subTest(msg='Berechnung von mitternacht(p, q) nicht korrekt für p = {} und q = {}.'.format(p, q)):
                self.runner(main.mitternacht.mitternacht, (p, q), self.assertAlmostEqualRelative)
